...
DeepSeek‑R1은 순수 강화학습(RL)을 통해 Reasoning 능력을 대폭 향상시킨 첫 번째 Reasoning 모델이다. 기존 DeepSeek‑V3의 학습 파이프라인과는 달리, R1은 SFT 단계 이전에 “R1‑Zero”라는 순수 RL 기반 Reasoning 모델을 먼저 구축하고, 이후 소량의 cold‑start 데이터를 활용해 SFT 및 추가 RL 과정을 거쳐 최종 모델로 완성된다. 이번 리뷰에서는 DeepSeek‑R1의 핵심 아이디어와 전체 흐름, 그리고 전통적인 학습 파이프라인과의 차별점을 중심으로 살펴본다.
...
- **DeepSeek‑R1의 Train Pipeline**
  - DeepSeek‑R1은 Pre‑training 단계 후, Base 모델을 기준으로 reasoning data sample을 이용한 RL로  **Interim Reasoning Model**(like R1 Zero) 모델을 먼저 구축한다.
...
- **형식**
  ...
  특정 태그(예: ‘<think> … </think>’ )를 이용하여 중간 추론 단계를 만든 후 해당 단계의 생성 토큰들을 다시 참고하여 답변한다.
...
DeepSeek‑R1의 파이프라인은 기존의 전통적인 방식과 달리, Base 모델에서 시작하여 바로 순수 강화학습(RL) 기반의 Interim Reasoning Model을 구축한 후, 이를 활용해 고품질 Reasoning 데이터(Cold Start Data)를 생성, 해당 데이터를 기반으로 SFT를 진행한 후, 추가 RL 단계를 통해 최종 R1 모델을 완성하는 방식이다.



### Interim Reasoning Model(like DeepSeek‑R1‑Zero)

- **소개**
  Interim Reasoning Model은 R1 pipeline의 첫 번째 RL 단계로, SFT 없이 Base 모델에서 직접 RL을 적용해 Reasoning 능력을 극대화한 모델이다.
- **만든 이유**
  RL 초기의 불안정한 cold start 단계를 피하기 위해, Base model을 긴 CoT 예시가 담긴 소량의 데이터로 fine-tune하여 RL actor로 활용하기 위함이다.
- **Base 모델에서 바로 Interim Reasoning Model을 만든 이유**
  초기 Cold Start 데이터가 부족한 상황에서, Base 모델이 이미 어느 정도 추론 역량을 갖추고 있음을 활용, RL 만으로도 강력한 Reasoning 능력을 이끌어낸다.
- **Interim Reasoning Model이 가능한 이유**
  다양한 평가 기준(예: 정답의 형식, 규칙 기반 검증을 통한 정확도 평가 등)을 통해 모델이 자체적으로 보상 신호를 얻으며 학습할 수 있기 때문에, SFT 데이터 없이도 RL이 가능하다.
- **Interim Reasoning Model의 약점**
  Reasoning 태스크에서는 뛰어난 성능을 보이나, 출력 형식이 불안정(예: 가독성 저하, 언어 혼합 현상)하여 사용자 친화적이지 못한 문제가 있다.
- **Interim Reasoning Model의 강점**
...
- **정의**
  Interim Reasoning Model을 더 유용하게 만들기 위해, 수천 개 규모의 reasoning 문제 예시(일부는 R1-Zero에서 생성 및 필터링)에 대해 supervised fine-tuning(SFT) 단계를 거친다. 이를 “cold start data”라고 부른다.



📌 *Cold Start Data 흐름 캡처*
👉 **[Cold Start Data 이미지 캡처 위치]**
...
- **학습 상세**
  + **SFT** 
    + Reasoning(Long CoT) 데이터는 약 300만 건의 예시를 생성한 후, 필터링을 통해 약 60만 건으로 정제된다.
  ...
  + **RL**
    PPO 및 GRPO 기법을 활용하여, 모델이 각 태스크(수학, 코딩, 논리 문제 등)에 대해 높은 정확도와 일관성을 갖도록 추가 RL을 진행합니다.
- **R1 Pipeline**
  - Pre-train(Empty model -> Base model)
  - Cold start data fine-tune(Base model -> Interim Reasoning Model)
  - SFT data 구축
    - 일반 instruction data 20만건
    - Interim reasoning model을 통해 생성 및 필터링한 long CoT data 60만건
  - SFT(Base Model -> SFT Checkpoint)
  - RL(SFT Checkpoint -> R1)
- **장점**
  - 전통적인 파이프라인은 대규모 SFT 데이터에서 CoT 데이터를 직접 수집해야하는 반면, R1 Pipeline은 Base model을 소량의 고품질 Cold Start Data를 순수 RL 통해 학습한 Interim Reasoning Model을 이용하여 막대한 양의 CoT 데이터를 생성하여 확보할 수 있다.
...
DeepSeek‑R1은 Interim Reasoning Model 활용해 효율적으로 reasoning 성능을 끌어올린 첫 케이스로, 다음과 같은 점에서 주목할 만하다.
